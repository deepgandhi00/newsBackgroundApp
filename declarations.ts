declare module 'react-native-sqlite-storage' {
  export interface Error {
    // this interface should match es5 error.
    name: string;
    message: string;
    stack?: string;
  }
  ...
  export interface Transaction {
    executeSql(
      statement: string,
      params?: any[],
      onSuccess?: (tx: Transaction, result: Result) => void,
      onError?: (tx: Transaction, error: Error) => void,
    ): void;
  }
  ...
  export interface Database {
    ...
    // ): void;
    sqlBatch(
      statements: Array<string | [string, any[]]>,
      onSuccess?: SuccessCallback,
      onError?: ErrorCallback,
    ): void;
    transaction(
